import { useEffect, useState } from "react";
import { ChevronDown, ChevronUp, Search, X } from "lucide-react";

import PageHeader from "../components/PageHeader";
import ConfirmModal from "../components/ConfirmModal";
import StatusBadge from "../components/StatusBadge";

import { useAuth } from "../contexts/AuthContext";

import {
  addChallenge,
  deleteChallenge,
  getChallengesByAdmin,
  updateChallenge,
} from "../services/challengeService";

import {
  getChallengeSubmissionsByAdmin,
  reviewChallengeSubmission,
} from "../services/challengeSubmissionService";

const challengeStatusOptions = [
  {
    value: "active",
    label: "Active",
  },
  {
    value: "inactive",
    label: "Inactive",
  },
];

const proofTypeOptions = [
  {
    value: "photo",
    label: "Photo Evidence",
  },
  {
    value: "photo_quantity",
    label: "Photo + Quantity",
  },
];

function Challenges() {
  const { currentAdmin } = useAuth();

  const [challenges, setChallenges] = useState([]);
  const [submissions, setSubmissions] = useState([]);

  const [searchTerm, setSearchTerm] = useState("");
  const [selectedStatusFilter, setSelectedStatusFilter] = useState("all");

  const [selectedChallenge, setSelectedChallenge] = useState(null);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);

  const [isFormModalOpen, setIsFormModalOpen] = useState(false);
  const [formMode, setFormMode] = useState("add");

  const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);
  const [isHistoryOpen, setIsHistoryOpen] = useState(false);

  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [points, setPoints] = useState("");
  const [status, setStatus] = useState("active");
  const [proofType, setProofType] = useState("photo");
  const [quantityRequired, setQuantityRequired] = useState("");

  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");

  useEffect(() => {
    async function fetchData() {
      if (!currentAdmin) return;

      try {
        setIsLoading(true);
        setErrorMessage("");

        const [challengesData, submissionsData] = await Promise.all([
          getChallengesByAdmin(currentAdmin),
          getChallengeSubmissionsByAdmin(currentAdmin),
        ]);

        setChallenges(challengesData);
        setSubmissions(submissionsData);
      } catch (error) {
        console.error(error);
        setErrorMessage(error.message || "Failed to load challenges.");
      } finally {
        setIsLoading(false);
      }
    }

    fetchData();
  }, [currentAdmin]);

  const filteredChallenges = challenges.filter((challenge) => {
    const keyword = searchTerm.toLowerCase();

    const matchSearch =
      challenge.title.toLowerCase().includes(keyword) ||
      challenge.description.toLowerCase().includes(keyword);

    const matchStatus =
      selectedStatusFilter === "all" ||
      challenge.status === selectedStatusFilter;

    return matchSearch && matchStatus;
  });

  function resetForm() {
    setTitle("");
    setDescription("");
    setPoints("");
    setStatus("active");
    setProofType("photo");
    setQuantityRequired("");
    setSelectedChallenge(null);
    setFormMode("add");
  }

  function handleAddChallenge() {
    resetForm();
    setFormMode("add");
    setIsFormModalOpen(true);
  }

  function handleEditChallenge(challenge) {
    setFormMode("edit");
    setSelectedChallenge(challenge);

    setTitle(challenge.title);
    setDescription(challenge.description);
    setPoints(challenge.points);
    setStatus(challenge.status);
    setProofType(challenge.proofType);
    setQuantityRequired(challenge.quantityRequired || "");

    setIsFormModalOpen(true);
  }

  function closeFormModal() {
    resetForm();
    setIsFormModalOpen(false);
  }

  async function handleSubmitChallenge(event) {
    event.preventDefault();

    if (!title.trim()) {
      alert("Challenge title is required");
      return;
    }

    if (!description.trim()) {
      alert("Challenge description is required");
      return;
    }

    if (!points || Number(points) <= 0) {
      alert("Points must be greater than 0");
      return;
    }

    if (proofType === "photo_quantity") {
      if (!quantityRequired || Number(quantityRequired) <= 0) {
        alert("Quantity required must be greater than 0");
        return;
      }
    }

    try {
      setIsSaving(true);

      const challengeData = {
        title,
        description,
        points: Number(points),
        status,
        proofType,
        quantityRequired:
          proofType === "photo_quantity" ? Number(quantityRequired) : null,
      };

      if (formMode === "edit") {
        await updateChallenge(selectedChallenge.id, challengeData);

        const updatedChallenges = challenges.map((challenge) => {
          if (challenge.id === selectedChallenge.id) {
            return {
              ...challenge,
              ...challengeData,
              proofRequired: true,
            };
          }

          return challenge;
        });

        setChallenges(updatedChallenges);
        closeFormModal();
        return;
      }

      const newChallengeId = await addChallenge(currentAdmin, challengeData);

      const newChallenge = {
        id: newChallengeId,
        ...challengeData,
        proofRequired: true,
        universityId: currentAdmin.universityId,
        universityName: currentAdmin.universityName,
        universityShortName: currentAdmin.universityShortName,
        departmentId: currentAdmin.departmentId,
        departmentName: currentAdmin.departmentName,
        createdBy: currentAdmin.id,
        createdAt: new Date(),
      };

      setChallenges([newChallenge, ...challenges]);
      closeFormModal();
    } catch (error) {
      console.error(error);
      alert("Failed to save challenge.");
    } finally {
      setIsSaving(false);
    }
  }

  function openDeleteModal(challenge) {
    setSelectedChallenge(challenge);
    setIsDeleteModalOpen(true);
  }

  function closeDeleteModal() {
    setSelectedChallenge(null);
    setIsDeleteModalOpen(false);
  }

  async function handleDeleteChallenge() {
    if (!selectedChallenge) return;

    try {
      await deleteChallenge(selectedChallenge.id);

      const updatedChallenges = challenges.filter(
        (challenge) => challenge.id !== selectedChallenge.id
      );

      setChallenges(updatedChallenges);
      closeDeleteModal();
    } catch (error) {
      console.error(error);
      alert("Failed to delete challenge.");
    }
  }

  function openDetailModal(challenge) {
    setSelectedChallenge(challenge);
    setIsHistoryOpen(false);
    setIsDetailModalOpen(true);
  }

  function closeDetailModal() {
    setSelectedChallenge(null);
    setIsHistoryOpen(false);
    setIsDetailModalOpen(false);
  }

  function getProofLabel(challenge) {
    if (challenge.proofType === "photo_quantity") {
      return `Photo + ${challenge.quantityRequired} items`;
    }

    return "Photo Evidence";
  }

  function getChallengeSubmissions(challengeId) {
    return submissions.filter((submission) => submission.challengeId === challengeId);
  }

  function getWaitingSubmissions(challengeId) {
    return getChallengeSubmissions(challengeId).filter(
      (submission) => submission.status === "submitted"
    );
  }

  function getHistorySubmissions(challengeId) {
    return getChallengeSubmissions(challengeId).filter(
      (submission) => submission.status !== "submitted"
    );
  }

  async function handleReviewSubmission(submissionId, newStatus) {
    if (!selectedChallenge || !currentAdmin) return;

    try {
      const pointsAwarded =
        newStatus === "approved" ? Number(selectedChallenge.points) : 0;

      const adminNote =
        newStatus === "approved"
          ? "Submission approved. Points awarded."
          : "Submission rejected. Evidence is not valid enough.";

      await reviewChallengeSubmission({
        submissionId,
        newStatus,
        pointsAwarded,
        adminNote,
        reviewedBy: currentAdmin.id,
      });

      const updatedSubmissions = submissions.map((submission) => {
        if (submission.id === submissionId) {
          return {
            ...submission,
            status: newStatus,
            pointsAwarded,
            adminNote,
            reviewedBy: currentAdmin.id,
            reviewedAt: new Date(),
          };
        }

        return submission;
      });

      setSubmissions(updatedSubmissions);
    } catch (error) {
      console.error(error);
      alert("Failed to review submission.");
    }
  }

  function getSubmissionSummary(challengeId) {
    const challengeSubmissions = getChallengeSubmissions(challengeId);

    const submittedCount = challengeSubmissions.filter(
      (submission) => submission.status === "submitted"
    ).length;

    const approvedCount = challengeSubmissions.filter(
      (submission) => submission.status === "approved"
    ).length;

    const rejectedCount = challengeSubmissions.filter(
      (submission) => submission.status === "rejected"
    ).length;

    return {
      total: challengeSubmissions.length,
      submitted: submittedCount,
      approved: approvedCount,
      rejected: rejectedCount,
    };
  }

  function formatDate(value) {
    if (!value) return "-";

    if (value?.toDate) {
      return value.toDate().toLocaleDateString("id-ID");
    }

    if (value instanceof Date) {
      return value.toLocaleDateString("id-ID");
    }

    return String(value);
  }

  return (
    <div>
      <PageHeader
        title="Challenges"
        description="Create challenges, view student submissions, and approve or reject evidence."
        actionLabel="Add Challenge"
        onAction={handleAddChallenge}
      />

      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-800">
              Challenge List
            </h2>
            <p className="text-sm text-slate-500">
              Showing challenges for {currentAdmin?.departmentName}.
            </p>
          </div>

          <div className="flex flex-col gap-3 md:flex-row">
            <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
              <Search size={16} className="text-slate-400" />

              <input
                type="text"
                placeholder="Search challenge..."
                value={searchTerm}
                onChange={(event) => setSearchTerm(event.target.value)}
                className="w-full bg-transparent text-sm outline-none placeholder:text-slate-400 md:w-56"
              />
            </div>

            <select
              value={selectedStatusFilter}
              onChange={(event) => setSelectedStatusFilter(event.target.value)}
              className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-600 outline-none transition focus:border-emerald-500"
            >
              <option value="all">All Status</option>

              {challengeStatusOptions.map((statusOption) => (
                <option key={statusOption.value} value={statusOption.value}>
                  {statusOption.label}
                </option>
              ))}
            </select>
          </div>
        </div>

        {errorMessage && (
          <div className="mb-5 rounded-2xl bg-red-50 p-4 text-sm font-medium text-red-700">
            {errorMessage}
          </div>
        )}

        {isLoading ? (
          <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-slate-500">
            Loading challenges...
          </div>
        ) : (
          <div className="overflow-x-auto rounded-2xl border border-slate-200">
            <table className="w-full min-w-[1000px] border-collapse text-left text-sm">
              <thead className="bg-slate-50 text-slate-500">
                <tr>
                  <th className="px-4 py-3 font-semibold">Challenge</th>
                  <th className="px-4 py-3 font-semibold">Points</th>
                  <th className="px-4 py-3 font-semibold">Proof</th>
                  <th className="px-4 py-3 font-semibold">Submissions</th>
                  <th className="px-4 py-3 font-semibold">Status</th>
                  <th className="px-4 py-3 font-semibold">Created At</th>
                  <th className="px-4 py-3 font-semibold">Action</th>
                </tr>
              </thead>

              <tbody className="divide-y divide-slate-200">
                {filteredChallenges.map((challenge) => {
                  const summary = getSubmissionSummary(challenge.id);

                  return (
                    <tr key={challenge.id} className="hover:bg-slate-50">
                      <td className="px-4 py-4">
                        <p className="font-medium text-slate-800">
                          {challenge.title}
                        </p>
                        <p className="mt-1 line-clamp-1 text-xs text-slate-500">
                          {challenge.description}
                        </p>
                      </td>

                      <td className="px-4 py-4 text-slate-600">
                        {challenge.points} pts
                      </td>

                      <td className="px-4 py-4 text-slate-600">
                        {getProofLabel(challenge)}
                      </td>

                      <td className="px-4 py-4">
                        <div className="space-y-1 text-xs text-slate-600">
                          <p>{summary.total} total</p>
                          <p>{summary.submitted} waiting review</p>
                        </div>
                      </td>

                      <td className="px-4 py-4">
                        <StatusBadge status={challenge.status} />
                      </td>

                      <td className="px-4 py-4 text-slate-600">
                        {formatDate(challenge.createdAt)}
                      </td>

                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          <button
                            type="button"
                            onClick={() => openDetailModal(challenge)}
                            className="rounded-xl border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-50"
                          >
                            Detail
                          </button>

                          <button
                            type="button"
                            onClick={() => handleEditChallenge(challenge)}
                            className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50"
                          >
                            Edit
                          </button>

                          <button
                            type="button"
                            onClick={() => openDeleteModal(challenge)}
                            className="rounded-xl border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-50"
                          >
                            Delete
                          </button>
                        </div>
                      </td>
                    </tr>
                  );
                })}

                {filteredChallenges.length === 0 && (
                  <tr>
                    <td
                      colSpan="7"
                      className="px-4 py-8 text-center text-slate-500"
                    >
                      No challenges found.
                    </td>
                  </tr>
                )}
              </tbody>
            </table>
          </div>
        )}
      </div>

      {isDetailModalOpen && selectedChallenge && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-3xl bg-white p-6 shadow-lg">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold text-slate-800">
                  Challenge Detail
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  Review all student submissions for this challenge.
                </p>
              </div>

              <button
                type="button"
                onClick={closeDetailModal}
                className="rounded-2xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-50"
              >
                <X size={18} />
              </button>
            </div>

            <div className="grid gap-5 lg:grid-cols-3">
              <div className="rounded-3xl bg-slate-50 p-5">
                <h3 className="font-bold text-slate-800">
                  {selectedChallenge.title}
                </h3>

                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {selectedChallenge.description}
                </p>

                <div className="mt-5 space-y-4">
                  <div>
                    <p className="text-xs font-semibold uppercase text-slate-400">
                      Points
                    </p>
                    <p className="mt-1 text-sm font-semibold text-slate-700">
                      {selectedChallenge.points} points
                    </p>
                  </div>

                  <div>
                    <p className="text-xs font-semibold uppercase text-slate-400">
                      Proof Type
                    </p>
                    <p className="mt-1 text-sm font-semibold text-slate-700">
                      {getProofLabel(selectedChallenge)}
                    </p>
                  </div>

                  <div>
                    <p className="text-xs font-semibold uppercase text-slate-400">
                      Status
                    </p>
                    <div className="mt-2">
                      <StatusBadge status={selectedChallenge.status} />
                    </div>
                  </div>

                  <div>
                    <p className="text-xs font-semibold uppercase text-slate-400">
                      Created At
                    </p>
                    <p className="mt-1 text-sm text-slate-600">
                      {formatDate(selectedChallenge.createdAt)}
                    </p>
                  </div>
                </div>
              </div>

              <div className="lg:col-span-2">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">
                      Waiting Review
                    </h3>

                    <p className="mt-1 text-sm text-slate-500">
                      Review new student submissions and approve valid evidence.
                    </p>
                  </div>

                  <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">
                    {getWaitingSubmissions(selectedChallenge.id).length} waiting
                  </span>
                </div>

                <div className="mt-4 space-y-4">
                  {getWaitingSubmissions(selectedChallenge.id).map(
                    (submission) => (
                      <div
                        key={submission.id}
                        className="rounded-3xl border border-slate-200 bg-white p-4"
                      >
                        <div className="flex flex-col gap-4 md:flex-row">
                          {submission.proofImageUrl ? (
                            <img
                              src={submission.proofImageUrl}
                              alt={submission.studentName}
                              className="h-40 w-full rounded-2xl object-cover md:w-48"
                            />
                          ) : (
                            <div className="flex h-40 w-full items-center justify-center rounded-2xl bg-slate-100 text-sm text-slate-400 md:w-48">
                              No photo
                            </div>
                          )}

                          <div className="flex-1">
                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                              <div>
                                <h4 className="font-bold text-slate-800">
                                  {submission.studentName}
                                </h4>
                                <p className="text-sm text-slate-500">
                                  {submission.studentEmail}
                                </p>
                              </div>

                              <StatusBadge status={submission.status} />
                            </div>

                            <div className="mt-4 grid gap-3 md:grid-cols-2">
                              <div>
                                <p className="text-xs font-semibold uppercase text-slate-400">
                                  Submitted At
                                </p>
                                <p className="mt-1 text-sm text-slate-600">
                                  {formatDate(submission.submittedAt)}
                                </p>
                              </div>

                              {selectedChallenge.proofType ===
                                "photo_quantity" && (
                                <div>
                                  <p className="text-xs font-semibold uppercase text-slate-400">
                                    Quantity
                                  </p>
                                  <p className="mt-1 text-sm text-slate-600">
                                    {submission.submittedQuantity} /{" "}
                                    {selectedChallenge.quantityRequired} items
                                  </p>
                                </div>
                              )}

                              <div>
                                <p className="text-xs font-semibold uppercase text-slate-400">
                                  Points Awarded
                                </p>
                                <p className="mt-1 text-sm text-slate-600">
                                  {submission.pointsAwarded} points
                                </p>
                              </div>
                            </div>

                            <div className="mt-4">
                              <p className="text-xs font-semibold uppercase text-slate-400">
                                Student Note
                              </p>
                              <p className="mt-1 text-sm leading-6 text-slate-600">
                                {submission.note}
                              </p>
                            </div>

                            <div className="mt-5 flex flex-wrap gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  handleReviewSubmission(
                                    submission.id,
                                    "approved"
                                  )
                                }
                                className="rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-emerald-700"
                              >
                                Approve
                              </button>

                              <button
                                type="button"
                                onClick={() =>
                                  handleReviewSubmission(
                                    submission.id,
                                    "rejected"
                                  )
                                }
                                className="rounded-xl bg-red-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-red-700"
                              >
                                Reject
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    )
                  )}

                  {getWaitingSubmissions(selectedChallenge.id).length === 0 && (
                    <div className="rounded-3xl border border-dashed border-slate-300 p-8 text-center">
                      <p className="font-semibold text-slate-700">
                        No submissions waiting for review.
                      </p>
                      <p className="mt-1 text-sm text-slate-500">
                        New student submissions will appear here.
                      </p>
                    </div>
                  )}
                </div>

                <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50">
                  <button
                    type="button"
                    onClick={() => setIsHistoryOpen(!isHistoryOpen)}
                    className="flex w-full items-center justify-between px-5 py-4 text-left"
                  >
                    <div>
                      <h3 className="font-bold text-slate-800">
                        Review History
                      </h3>
                      <p className="mt-1 text-sm text-slate-500">
                        Approved and rejected submissions.
                      </p>
                    </div>

                    <div className="flex items-center gap-3">
                      <span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold text-slate-600">
                        {getHistorySubmissions(selectedChallenge.id).length}{" "}
                        history
                      </span>

                      {isHistoryOpen ? (
                        <ChevronUp size={18} className="text-slate-500" />
                      ) : (
                        <ChevronDown size={18} className="text-slate-500" />
                      )}
                    </div>
                  </button>

                  {isHistoryOpen && (
                    <div className="max-h-96 space-y-4 overflow-y-auto border-t border-slate-200 p-4">
                      {getHistorySubmissions(selectedChallenge.id).map(
                        (submission) => (
                          <div
                            key={submission.id}
                            className="rounded-3xl border border-slate-200 bg-slate-100 p-4 opacity-90"
                          >
                            <div className="flex flex-col gap-4 md:flex-row">
                              {submission.proofImageUrl ? (
                                <img
                                  src={submission.proofImageUrl}
                                  alt={submission.studentName}
                                  className="h-32 w-full rounded-2xl object-cover grayscale md:w-40"
                                />
                              ) : (
                                <div className="flex h-32 w-full items-center justify-center rounded-2xl bg-slate-200 text-sm text-slate-400 md:w-40">
                                  No photo
                                </div>
                              )}

                              <div className="flex-1">
                                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                  <div>
                                    <h4 className="font-bold text-slate-700">
                                      {submission.studentName}
                                    </h4>
                                    <p className="text-sm text-slate-500">
                                      {submission.studentEmail}
                                    </p>
                                  </div>

                                  <StatusBadge status={submission.status} />
                                </div>

                                <div className="mt-4 grid gap-3 md:grid-cols-2">
                                  <div>
                                    <p className="text-xs font-semibold uppercase text-slate-400">
                                      Submitted At
                                    </p>
                                    <p className="mt-1 text-sm text-slate-600">
                                      {formatDate(submission.submittedAt)}
                                    </p>
                                  </div>

                                  {selectedChallenge.proofType ===
                                    "photo_quantity" && (
                                    <div>
                                      <p className="text-xs font-semibold uppercase text-slate-400">
                                        Quantity
                                      </p>
                                      <p className="mt-1 text-sm text-slate-600">
                                        {submission.submittedQuantity} /{" "}
                                        {selectedChallenge.quantityRequired}{" "}
                                        items
                                      </p>
                                    </div>
                                  )}

                                  <div>
                                    <p className="text-xs font-semibold uppercase text-slate-400">
                                      Points Awarded
                                    </p>
                                    <p className="mt-1 text-sm text-slate-600">
                                      {submission.pointsAwarded} points
                                    </p>
                                  </div>
                                </div>

                                <div className="mt-4">
                                  <p className="text-xs font-semibold uppercase text-slate-400">
                                    Student Note
                                  </p>
                                  <p className="mt-1 text-sm leading-6 text-slate-600">
                                    {submission.note}
                                  </p>
                                </div>

                                {submission.adminNote && (
                                  <div className="mt-4 rounded-2xl bg-white/70 p-3">
                                    <p className="text-xs font-semibold uppercase text-slate-400">
                                      Admin Note
                                    </p>
                                    <p className="mt-1 text-sm leading-6 text-slate-600">
                                      {submission.adminNote}
                                    </p>
                                  </div>
                                )}
                              </div>
                            </div>
                          </div>
                        )
                      )}

                      {getHistorySubmissions(selectedChallenge.id).length ===
                        0 && (
                        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-center">
                          <p className="font-semibold text-slate-700">
                            No review history yet.
                          </p>
                          <p className="mt-1 text-sm text-slate-500">
                            Approved or rejected submissions will appear here.
                          </p>
                        </div>
                      )}
                    </div>
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>
      )}

      {isFormModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-lg">
            <h2 className="text-xl font-bold text-slate-800">
              {formMode === "edit" ? "Edit Challenge" : "Add Challenge"}
            </h2>

            <p className="mt-2 text-sm text-slate-500">
              {formMode === "edit"
                ? "Update challenge information."
                : `Create a new challenge for ${currentAdmin?.departmentName}.`}
            </p>

            <form onSubmit={handleSubmitChallenge} className="mt-6 space-y-4">
              <div>
                <label className="text-sm font-semibold text-slate-700">
                  Challenge Title
                </label>

                <input
                  type="text"
                  value={title}
                  onChange={(event) => setTitle(event.target.value)}
                  placeholder="Example: Setor 5 Botol Plastik"
                  className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-500"
                />
              </div>

              <div>
                <label className="text-sm font-semibold text-slate-700">
                  Description
                </label>

                <textarea
                  value={description}
                  onChange={(event) => setDescription(event.target.value)}
                  placeholder="Explain what students need to do."
                  rows="4"
                  className="mt-2 w-full resize-none rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-500"
                />
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="text-sm font-semibold text-slate-700">
                    Points
                  </label>

                  <input
                    type="number"
                    value={points}
                    onChange={(event) => setPoints(event.target.value)}
                    placeholder="Example: 30"
                    min="1"
                    className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-500"
                  />
                </div>

                <div>
                  <label className="text-sm font-semibold text-slate-700">
                    Status
                  </label>

                  <select
                    value={status}
                    onChange={(event) => setStatus(event.target.value)}
                    className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-500"
                  >
                    {challengeStatusOptions.map((statusOption) => (
                      <option
                        key={statusOption.value}
                        value={statusOption.value}
                      >
                        {statusOption.label}
                      </option>
                    ))}
                  </select>
                </div>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="text-sm font-semibold text-slate-700">
                    Proof Type
                  </label>

                  <select
                    value={proofType}
                    onChange={(event) => setProofType(event.target.value)}
                    className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-500"
                  >
                    {proofTypeOptions.map((proofOption) => (
                      <option
                        key={proofOption.value}
                        value={proofOption.value}
                      >
                        {proofOption.label}
                      </option>
                    ))}
                  </select>
                </div>

                {proofType === "photo_quantity" && (
                  <div>
                    <label className="text-sm font-semibold text-slate-700">
                      Quantity Required
                    </label>

                    <input
                      type="number"
                      value={quantityRequired}
                      onChange={(event) =>
                        setQuantityRequired(event.target.value)
                      }
                      placeholder="Example: 5"
                      min="1"
                      className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-emerald-500"
                    />
                  </div>
                )}
              </div>

              <div className="rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-700">
                <p className="font-semibold">Proof guide</p>
                <p className="mt-1">
                  Photo Evidence means students upload one proof photo. Photo +
                  Quantity means students upload a photo and enter the number of
                  items completed.
                </p>
              </div>

              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={closeFormModal}
                  disabled={isSaving}
                  className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:bg-slate-100"
                >
                  Cancel
                </button>

                <button
                  type="submit"
                  disabled={isSaving}
                  className="rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                >
                  {isSaving
                    ? "Saving..."
                    : formMode === "edit"
                    ? "Update Challenge"
                    : "Save Challenge"}
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      <ConfirmModal
        isOpen={isDeleteModalOpen}
        title="Delete Challenge"
        message={`Are you sure you want to delete "${
          selectedChallenge?.title || "this challenge"
        }"? This action cannot be undone.`}
        confirmLabel="Delete"
        cancelLabel="Cancel"
        onConfirm={handleDeleteChallenge}
        onCancel={closeDeleteModal}
      />
    </div>
  );
}

export default Challenges;